...
def pandas_search(condition_data: list[dict], detail_info: list[dict]) -> list[dict]:
    ...
    condition_info_df = pd.merge(pd.DataFrame(condition_data), info_df, on='place_id', how='inner')
    ...
    for period_value in filtered_df['period'].unique():
        period_df = filtered_df[filtered_df['period'] == period_value]  # 根據 period 進行篩選
        top_tier = period_df.nlargest(50, 'rating')[['place_id', 'place_name', 'rating', 'lon', 'lat', 'label_type', 'label', 'hours', 'period', 'url']]
